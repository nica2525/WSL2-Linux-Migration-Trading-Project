...
class VolatilityAnalyzer:
    ...
    def get_volatility_multiplier(self, volatility_level: str) -> float:
        """ボラティリティ倍率"""
        multipliers = {"low": 0.5, "medium": 1.0, "high": 1.5, "extreme": 2.0}
...
class MarketEnvironmentDetector:
    ...
    def detect_market_environment(
        self, price_data: List[Dict], current_time: datetime
    ) -> MarketEnvironment:
        """市場環境検知"""
        ...
        analyzer.classify_volatility(current_atr, historical_atr)
...
class AdaptiveRiskManager:
    ...
    def calculate_adaptive_risk_parameters(
        self,
        ...
        # 市場環境検知
        market_env = self.environment_detector.detect_market_environment(
            price_data, current_time
        )
        ...
        volatility_level = self.volatility_analyzer.classify_volatility(
        ...
        news_adj = self.environment_adjustments["news"][
            "high_impact" if market_env.is_high_impact_news else "normal"
        ]
        ...
        adjusted_stop_loss = (
            base_stop_loss * vol_adj["stop"] * session_adj["stop"] * news_adj["stop"]
        )
        ...
        return RiskParameters(
            ...
            volatility_multiplier=self.volatility_analyzer.get_volatility_multiplier(
                volatility_level
            ),
            # 新強化項目
    ...
    def calculate_advanced_risk_metrics(
        self, trade_history: List[Dict], current_positions: List[Dict] = None
    ) -> Dict:
        """高度なリスク指標計算"""
    ...
    def _calculate_heat_index(self, trade_history: List[Dict]) -> float:
        ...
        historical_trades = (
            trade_history[-40:-20] if len(trade_history) >= 40 else trade_history[:-20]
        )

        recent_avg = sum(t.get("pnl", 0) for t in recent_trades) / len(recent_trades)
        historical_avg = (
            sum(t.get("pnl", 0) for t in historical_trades) / len(historical_trades)
    ...
    def _calculate_correlation_risk(self, current_positions: List[Dict]) -> float:
        ...
        # 簡易相関（同方向ポジション比率）
        long_positions = sum(
            1 for p in current_positions if p.get("direction") == "BUY"
        )
        short_positions = sum(
            1 for p in current_positions if p.get("direction") == "SELL"
        )
...
def main():
    ...
    # リスク管理システム初期化
    base_params = {"stop_atr": 1.3, "profit_atr": 2.5, "atr_period": 14}
