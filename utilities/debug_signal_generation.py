...
def debug_signal_generation():
    ...
    for i in range(100, min(600, len(h1_data)), 50):
        try:
            ...
            signal = generate_manual_signal(
                h1_data, h4_data, i, base_params, current_time
            )
    ...
    if trading_rate == 0:
        ...
        for i in range(100, min(400, len(h1_data)), 50):
            signal = generate_manual_signal(
                h1_data, h4_data, i, relaxed_params, h1_data[i]["datetime"]
            )
...
def generate_manual_signal(h1_data, h4_data, current_index, params, current_time):
    ...
    if current_price > h4_high and current_price > h1_high:
        return {
            "action": "BUY",
            "breakout_strength": (current_price - max(h4_high, h1_high))
            / current_price
            * 10000,
            "h1_high": h1_high,
            "h4_high": h4_high,
        ...
        return {
            "action": "SELL",
            "breakout_strength": (min(h4_low, h1_low) - current_price)
            / current_price
            * 10000,
            "h1_low": h1_low,
            "h4_low": h4_low,
