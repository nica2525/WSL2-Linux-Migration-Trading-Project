...
class CostResistantStrategy:
    ...
    def generate_cost_resistant_signal(self, 
                                     mtf_data: MultiTimeframeData,
                                     current_time: datetime) -> Optional[CostResistantSignal]:
        """コスト耐性シグナル生成"""
        ...
        if len(h1_data) < self.cost_resistance_params['trend_ma_period'] or len(h4_data) < 50:
            return None
        ...
        trend_filter_passed, trend_strength = self._check_trend_filter(h1_data, base_signal)
        if not trend_filter_passed:
            self.signals_filtered_trend += 1
            return None
        
        # 3. 利益期待フィルター: 十分な利益期待のみ
        profit_filter_passed, expected_profit_pips = self._check_profit_filter(h1_data, base_signal)
        if not profit_filter_passed:
            self.signals_filtered_profit += 1
            return None
        
        # 4. シグナル品質評価
        confidence = self._evaluate_signal_quality(atr_multiple, trend_strength, expected_profit_pips)
        ...
        stop_loss, take_profit = self._calculate_optimized_levels(h1_data, base_signal, expected_profit_pips)
    ...
    def _check_trend_filter(self, h1_data: List, base_signal: Dict) -> Tuple[bool, float]:
        ...
        min_strength = self.cost_resistance_params['min_trend_strength']
        ...
        # トレンド強度計算（スケール調整）
        price_vs_long = abs(current_price - ma_long) / ma_long * 100 if ma_long > 0 else 0  # パーセント変換
        short_vs_long = abs(ma_short - ma_long) / ma_long * 100 if ma_long > 0 else 0  # パーセント変換
    ...
    def _check_profit_filter(self, h1_data: List, base_signal: Dict) -> Tuple[bool, float]:
    ...
    def _evaluate_signal_quality(self, atr_multiple: float, trend_strength: float, 
                                expected_profit_pips: float) -> str:
        """シグナル品質評価"""
        score = 0
    ...
    def _calculate_optimized_levels(self, h1_data: List, base_signal: Dict, 
                                  expected_profit_pips: float) -> Tuple[float, float]:
        """最適化されたストップ・利確レベル計算"""
    ...
    def get_statistics(self) -> Dict:
        """統計情報取得"""
        total_filtered = (self.signals_filtered_atr + 
        ...
        return {
            ...
            'approval_rate': self.signals_approved / self.signals_generated if self.signals_generated > 0 else 0,
            'filter_effectiveness': total_filtered / self.signals_generated if self.signals_generated > 0 else 0
        }
