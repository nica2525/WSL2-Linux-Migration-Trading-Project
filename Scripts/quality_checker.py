...
class QualityChecker:
        ...
        for py_file in py_files:
            ...
            try:
                ...
                for issue_type, config in self.scan_patterns.items():
                    matches = re.finditer(
                        config["pattern"], content, re.IGNORECASE | re.MULTILINE
                    )
    ...
    def generate_quality_report(self):
        ...
        # 前回からの変化を検出
        current_signatures = {
            f"{issue['file']}:{issue['line']}:{issue['type']}" for issue in issues
        }
        previous_signatures = {
        ...
        new_issues = [
            issue
            for issue in issues
            if f"{issue['file']}:{issue['line']}:{issue['type']}"
        ...
        if report["summary"]["high_severity"] > 0:
            ...
            for issue in report["all_issues"]:
                if issue["severity"] == "HIGH":
                    confidence_badge = (
                        "🔴" if issue.get("confidence", 0.8) > 0.9 else "🟡"
                    )
                    context = issue.get("context", {})
                    func_info = (
                        f" [{context.get('function', 'unknown')}]"
                        if context.get("function") != "unknown"
                        else ""
                    )
                    briefing.append(
                        f"   {confidence_badge} {issue['file']}:{issue['line']} - {issue['description']}{func_info}"
                    )
    ...
    def generate_detailed_analysis(self):
        ...
        analysis["file_statistics"] = {
            file: {
                ...
                "avg_confidence": sum(i.get("confidence", 0.8) for i in issues)
                / len(issues)
                if issues
                else 0,
            }
    ...
    def create_improvement_suggestions(self):
        ...
        frequent_patterns = sorted(
            analysis["pattern_frequency"].items(), key=lambda x: x[1], reverse=True
        )[:2]
