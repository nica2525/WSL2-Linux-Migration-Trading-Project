...
class RealityEnhancedWFA:
    ...
    def apply_transaction_costs(self, trades_df: pd.DataFrame) -> pd.DataFrame:
        ...
        trades_df["net_return_after_costs"] = (
            trades_df["raw_return"] - trades_df["transaction_cost"]
        )
    ...
    def simulate_slippage(self, trades_df: pd.DataFrame) -> pd.DataFrame:
        ...
        trades_df["net_return_after_slippage"] = (
            trades_df["net_return_after_costs"] - trades_df["slippage_cost"]
        )
    ...
    def compute_drawdown(
        ...
        for _i, dd in enumerate(drawdown):
            if dd < 0:
    ...
    def execute_reality_enhanced_wfa(self):
        ...
        # 統計的分析
        statistical_results = self._perform_enhanced_statistical_analysis(
            enhanced_results
        )

        # 最終判定
        final_judgment = self._perform_reality_judgment(
    ...
    def _execute_simple_wfa(self, raw_data):
        ...
        try:
            ...
            for i, (is_data, oos_data) in enumerate(folds, 1):
                ...
                oos_pf, oos_trades, oos_return = self._calculate_performance(
                    oos_signals
                )
                ...
                oos_win_rate = (
                    self._calculate_win_rate(oos_signals) if oos_signals else 0
                )
    ...
    def _apply_reality_analysis(self, wfa_results):
        ...
        print("\n🔬 リアリティ分析適用:")
        print(
            f"   取引コスト: {self.reality_params['spread_pips']}+{self.reality_params['commission_pips']} pips"
        )
        ...
        for result in wfa_results:
            ...
            if result["oos_trades"] > 0:
                ...
                cost_impact = (
                    df["net_return_after_slippage"].iloc[0] / result["oos_return"]
                )
            ...
            enhanced_result.update(
                {
                    ...
                    "total_cost": df["transaction_cost"].iloc[0]
                    + df["slippage_cost"].iloc[0],
    ...
    def _perform_enhanced_statistical_analysis(self, enhanced_results):
        ...
        if len(reality_pfs) > 1:
            t_stat = (avg_reality_pf - 1.0) / (
                np.std(reality_pfs, ddof=1) / np.sqrt(len(reality_pfs))
            )
            # 簡易p値計算
        ...
        return {
            ...
            "avg_max_drawdown": np.mean(
                [r["max_drawdown_pct"] for r in enhanced_results]
            ),
        }
        ...
        for i in range(signal_points):
            ...
            if idx < len(mtf_data.raw_data):
                ...
                if signal and signal.get("action") in ["BUY", "SELL"]:
                    # 実際の価格変動に基づくリターン計算
                    current_bar = mtf_data.raw_data[idx]
                    entry_price = current_bar["open"]  # 次バー始値想定

                    # 短期間（20バー後）の価格変動をリターンとして使用
                    exit_idx = min(idx + 20, len(mtf_data.raw_data) - 1)
                    exit_bar = mtf_data.raw_data[exit_idx]
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(
            f"   実用性判定: {'✅ 合格' if result['final_judgment']['reality_viability'] else '❌ 不合格'}"
        )
    else:
        print("❌ WFA実行失敗")
