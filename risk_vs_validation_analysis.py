...
def analyze_risk_vs_validation_quality():
    ...
    print("✅ データ準備完了")
    print(
        f"   期間: {test_period_start.strftime('%Y-%m-%d')} - {test_period_end.strftime('%Y-%m-%d')}"
    )
    ...
    for level_name, config in risk_levels.items():
        ...
        try:
            ...
            if signals_count > 0:
                ...
                statistical_power = (
                    "高"
                    if estimated_annual_trades >= 100
                    else ("中" if estimated_annual_trades >= 50 else "低")
                )
            else:
            ...
            print(f"   ❌ エラー: {str(e)}")
            results[level_name] = {"error": str(e), "validation_feasibility": "不可能"}
...
def analyze_recommendations(results):
    ...
    for level, data in feasible_levels.items():
        ...
        power_score = {"高": 3, "中": 2, "低": 1, "なし": 0}.get(
            data["statistical_power"], 0
        )
