...
class RealityCostAnalyzer:
    ...
    def calculate_reality_pf(self, original_pf: float, raw_return: float, 
                           final_return: float) -> float:
        """リアリティPF計算"""
    ...
    def analyze_wfa_results(self, wfa_results_file: str) -> Dict:
        ...
        # 既存結果の読み込み
        try:
            with open(wfa_results_file, 'r') as f:
                data = json.load(f)
        ...
        for result in wfa_results:
            ...
            # 総コスト計算
            total_cost = cost_result['transaction_cost'] + slippage_result['slippage_cost']
            cost_impact_pct = (total_cost / abs(oos_return)) * 100 if oos_return != 0 else 0
        ...
        # 最終判定
        final_judgment = self._perform_final_judgment(statistical_results, enhanced_results)
    ...
    def _perform_final_judgment(self, statistical_results, enhanced_results):
        ...
        criteria = {
            ...
            "cost_impact_acceptable": statistical_results["avg_cost_impact_pct"] <= 10.0,
    ...
    def display_results_summary(self, result):
        ...
        print(f"   統計的有意性: {'✅ あり' if stats['statistical_significance'] else '❌ なし'} (p={stats['p_value']:.3f})")
        print(f"   一貫性: {stats['consistency_ratio']*100:.0f}% ({stats['positive_folds']}/{stats['total_folds']})")
        print(f"   平均コスト影響: {stats['avg_cost_impact_pct']:.1f}%")
        
        print(f"\n🏆 最終判定:")
        print(f"   実用性: {'✅ 合格' if judgment['reality_viability'] else '❌ 不合格'}")
        print(f"   基準達成: {judgment['criteria_passed']}/{judgment['criteria_total']} ({judgment['success_rate']*100:.0f}%)")
