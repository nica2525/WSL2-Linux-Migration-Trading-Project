...
class ShortTermComprehensiveEvaluation:
    ...
    def __init__(self):
        ...
        self.results = {
            ...
            'full_data_verification': self._load_json_safe('full_data_verification_results.json'),
            'extended_period_wfa': self._load_json_safe('extended_period_wfa_results.json'),
            'market_environment_validation': self._load_json_safe('market_environment_validation_results.json')
        }
    ...
    def run_comprehensive_evaluation(self):
        ...
        self._save_comprehensive_evaluation(
            stage_evaluations, key_findings, learning_outcomes, 
            strategy_assessment, future_directions, overall_assessment
        )
    ...
    def _evaluate_each_stage(self):
        ...
        if self.results['minimal_wfa']:
            ...
            evaluations['minimal_wfa'] = {
                ...
                'total_trades': minimal_stats['avg_oos_trades'] * minimal_stats['total_folds'],
                'evaluation': 'EXCELLENT' if minimal_stats['statistical_significance'] else 'GOOD',
        ...
        if self.results['full_data_verification']:
            full_comparison = self.results['full_data_verification']['comparison_analysis']
            reliability = self.results['full_data_verification']['reliability_assessment']
            evaluations['full_data_verification'] = {
                'status': 'COMPLETED',
                'reliability_level': reliability['reliability'],
                'pf_consistency': full_comparison['overall_comparison']['pf_consistency'],
                'significance_maintained': full_comparison['overall_comparison']['significance_maintained'],
        ...
        if self.results['market_environment_validation']:
            market_adaptability = self.results['market_environment_validation']['adaptability_assessment']
            evaluations['market_environment_validation'] = {
                ...
                'environments_tested': len(self.results['market_environment_validation']['environment_results']),
                'evaluation': market_adaptability['adaptability'],
                'key_achievement': '市場環境依存性の定量的確認'
            }
            print(f"   市場環境検証: {evaluations['market_environment_validation']['evaluation']}")
    ...
    def _integrate_key_findings(self):
        ...
        if self.results['minimal_wfa'] and self.results['extended_period_wfa']:
            minimal_pf = self.results['minimal_wfa']['statistical_results']['avg_oos_pf']
            extended_pf = self.results['extended_period_wfa']['statistical_analysis']['avg_oos_pf']
        ...
        # 市場環境に関する発見
        if self.results['market_environment_validation']:
            market_results = self.results['market_environment_validation']['environment_results']
            pf_values = []
            for env_name, env_data in market_results.items():
    ...
    def _assess_strategy_realistically(self):
        ...
        if self.results['market_environment_validation']:
            ...
            cross_analysis = self.results['market_environment_validation']['cross_environment_analysis']
            if cross_analysis and cross_analysis['cross_analysis']['performance_stability'] == 'LOW':
    ...
    def _render_overall_assessment(self, stage_evals, findings, learning, strategy):
        ...
        # 段階別成功率
        completed_stages = sum(1 for stage in stage_evals.values() if stage['status'] == 'COMPLETED')
        ...
        strategy_score = 0.3 if strategy['overall_viability'] == 'CONDITIONALLY_VIABLE' else 0.1
    ...
    def _save_comprehensive_evaluation(self, stage_evals, findings, learning, strategy, future, overall):
        """総合評価結果保存"""
        evaluation_data = {
...
def main():
    ...
    try:
        ...
        if comprehensive_results:
            ...
            print(f"   次の焦点: {comprehensive_results['future_directions']['recommended_priority']}")
        else:
            print(f"\n⚠️ 総合評価で問題が発生")
