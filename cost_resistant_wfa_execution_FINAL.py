...
# ChatGPT提案: ステートレス並列処理関数
def _backtest_scenario_worker(args):
    """ステートレス並列ワーカー関数（ChatGPT根本修正版）"""
    params, raw_data, base_params = args
    
    try:
        # 独立したパラメータセット作成
        worker_params = base_params.copy()
        worker_params['spread_pips'] = params['spread_pips']
        worker_params['commission_pips'] = params['commission_pips']
        
        # 戦略インスタンス作成
        strategy = CostResistantStrategy(worker_params)
        ...
        for fold_id in range(1, 6):
            ...
            # 統一バックテストエンジン使用
            oos_result = _execute_unified_backtest(test_data, fold_id, strategy, worker_params)
            if oos_result:
...
def _execute_unified_backtest(test_data: List, fold_id: int, strategy, params: Dict) -> Dict:
    """統一バックテストエンジン（CostResistantStrategy完全活用版）"""
    try:
        # ポジション管理
        open_positions = []
        closed_trades = []
        ...
        strategy.signals_filtered_atr = 0
        strategy.signals_filtered_trend = 0
        strategy.signals_filtered_profit = 0
        strategy.signals_approved = 0
        ...
        for i in range(50, len(test_data)):
            ...
            for pos_idx, position in enumerate(open_positions):
                ...
                if trade_result:
                    ...
                    # コスト調整
                    trade_cost = (params['spread_pips'] + params['commission_pips']) * 10
            ...
            # 2. 新規シグナル判定（CostResistantStrategy完全活用）
            can_enter = (
                len(open_positions) == 0 and
                not bar_had_exit and
                _can_enter_new_trade_unified(current_time, closed_trades)
            )
            
            if can_enter:
                historical_data = test_data[:i+1]
                signal = _generate_signal_unified(historical_data, current_time, strategy, track_stats=True)
        ...
        if open_positions:
            ...
            for position in open_positions:
                pnl = ((final_price - position.entry_price) if position.direction == 'BUY' else 
                       (position.entry_price - final_price)) * 100000
        ...
        # ドローダウン計算
        raw_drawdown = _compute_drawdown_unified(equity_curve)
        cost_adjusted_drawdown = _compute_drawdown_unified(cost_adjusted_equity_curve)
        ...
        return {
            ...
            'drawdown_analysis': {
                ...
                'cost_adjusted_drawdown': cost_adjusted_drawdown
            }
        }
        
    except Exception as e:
        print(f"   ⚠️ Unified backtest error: {e}")
        return None

def _can_enter_new_trade_unified(current_time: datetime, closed_trades: List) -> bool:
    """新規取引可能判定（統一版）"""
    if not closed_trades:
        return True
...
def _generate_signal_unified(historical_data: List, current_time: datetime, strategy, track_stats: bool = False) -> Optional[Dict]:
    """統一シグナル生成（CostResistantStrategy完全活用版・_generate_signal_safeと完全同一）"""
    try:
        ...
        # フィルタリング適用（CostResistantStrategy完全活用）
        if track_stats:
            strategy.signals_generated += 1
        
        # ATRフィルター
        atr_passed, atr_multiple = strategy._check_atr_filter(historical_data, base_signal)
        if not atr_passed:
            if track_stats:
                strategy.signals_filtered_atr += 1
            return None
        
        # トレンドフィルター
        trend_passed, trend_strength = strategy._check_trend_filter(historical_data, base_signal)
        if not trend_passed:
            if track_stats:
                strategy.signals_filtered_trend += 1
            return None
        
        # 利益フィルター
        profit_passed, expected_profit = strategy._check_profit_filter(historical_data, base_signal)
        if not profit_passed:
            if track_stats:
                strategy.signals_filtered_profit += 1
            return None
        
        # 承認
        if track_stats:
            strategy.signals_approved += 1
        
        # ストップ・利確計算
        stop_loss, take_profit = strategy._calculate_optimized_levels(
...
def _compute_drawdown_unified(equity_curve: List) -> Dict:
    """ドローダウン計算（統一版）"""
    if len(equity_curve) < 2:
        return {'max_drawdown_pct': 0, 'max_drawdown_amount': 0, 'max_drawdown_periods': 0, 'is_acceptable': True}
    ...
    return {
        ...
        'is_acceptable': bool(max_drawdown_pct <= 0.10)
    }
...
class CostResistantWFAExecutionFinal:
    ...
    def execute_sensitivity_analysis_parallel(self) -> Dict:
        """感度分析自動化システム（並列処理版 - ChatGPT統一エンジン適用）"""
        ...
        # 並列実行（ChatGPT統一エンジン使用）
        with ProcessPoolExecutor(max_workers=cpu_count) as executor:
            # 各シナリオをワーカーに投入（統一ワーカー関数使用）
            future_to_params = {}
            for i, params in enumerate(sensitivity_params):
                args = (params, raw_data, self.base_params)
                future = executor.submit(_backtest_scenario_worker, args)
            ...
            for future in as_completed(future_to_params):
                ...
                try:
                    ...
                    if result and result['scenario_success']:
    ...
    def execute_final_wfa(self, silent_mode: bool = False) -> Dict:
        ...
        for fold_id in range(1, 6):
            ...
            # 現実的バックテスト実行
            # 統一エンジン使用（完全にCostResistantStrategy活用）
            oos_result = _execute_unified_backtest(test_data, fold_id, self.strategy, self.base_params)
    ...
    def _execute_realistic_backtest(self, test_data: List, fold_id: int) -> Dict:
        """現実的バックテスト（統一エンジンに統合済み・非推奨）"""
        # このメソッドは統一エンジンに統合されました。
        # 代わりに_execute_unified_backtestを使用してください。
        print(f"   ⚠️ 統一エンジン使用を推奨: _execute_unified_backtest")
        return _execute_unified_backtest(test_data, fold_id, self.strategy, self.base_params)
        try:
            # 状態リセット
            self.open_positions = []
            self.closed_trades = []
    ...
    def _generate_signal_safe(self, historical_data: List, current_time: datetime) -> Optional[Dict]:
        """安全なシグナル生成（統一エンジンに統合済み・非推奨）"""
        # このメソッドは統一エンジンに統合されました。
        # 代わりに_generate_signal_unifiedを使用してください。
        return _generate_signal_unified(historical_data, current_time, self.strategy, track_stats=True)
        try:
            if len(historical_data) < 50:
                return None
    ...
    def _generate_sensitivity_report(self, sensitivity_results: List) -> Dict:
        ...
        for result in sensitivity_results:
            # 並列処理結果の構造に対応
            if 'wfa_results' in result:
                # WFA結果から統計を計算
                wfa_results = result['wfa_results']
                if wfa_results:
                    avg_pf = sum(r.get('profit_factor', 0) for r in wfa_results) / len(wfa_results)
                    avg_return = sum(r.get('total_return', 0) for r in wfa_results) / len(wfa_results)
                    profitable_folds = sum(1 for r in wfa_results if r.get('profit_factor', 0) > 1.0)
                    total_folds = len(wfa_results)
                    consistency = profitable_folds / total_folds if total_folds > 0 else 0
                    ...
                    performance_comparison.append({
                        ...
                        'viable_strategy': bool(avg_pf > 1.0 and consistency > 0.6)
                    })
                ...
                performance_comparison.append({
                    ...
                    'viable_strategy': result.get('drawdown_risk', {}).get('is_strategy_viable', False)
                })
        
        # コスト耐性評価（並列結果構造に対応）
        cost_resistance_levels = []
        for result in sensitivity_results:
            # 並列処理結果は異なる構造なので、基本的な評価を実行
            if 'wfa_results' in result:
                wfa_results = result['wfa_results']
                if wfa_results:
                    avg_return = sum(r.get('total_return', 0) for r in wfa_results) / len(wfa_results)
                    ...
                    cost_resistance_levels.append({
                        'scenario': result['scenario'],
                        'cost_reduction_impact': 0.0,  # 並列版では詳細コスト分析未実装
                        'net_profitability': float(avg_return),
                        'risk_acceptable': bool(avg_pf > 1.0)
                    })
            elif 'cost_impact' in result:
                # 従来形式の処理
        ...
        for r in sensitivity_results:
            if 'wfa_results' in r:
                wfa_results = r['wfa_results']
                if wfa_results:
                    avg_pf = sum(res.get('profit_factor', 0) for res in wfa_results) / len(wfa_results)
                    if avg_pf > 1.0:
                        viable_scenarios.append(r)
            elif 'drawdown_risk' in r and r['drawdown_risk']['is_strategy_viable']:
                viable_scenarios.append(r)
        ...
        return {
            ...
            'strategy_viability': {
                ...
                'viability_rate': float(len(viable_scenarios) / len(sensitivity_results)),
                'recommended_environment': viable_scenarios[0]['scenario'] if viable_scenarios else 'None',
                'critical_cost_threshold': float(self._find_cost_threshold(sensitivity_results))
            },
            'risk_summary': {
                'best_scenario': performance_comparison[0]['scenario'] if performance_comparison else 'Unknown',
                'worst_scenario': performance_comparison[-1]['scenario'] if performance_comparison else 'Unknown',
                'avg_risk_violations': 0.0  # 並列版では詳細リスク分析未実装
            }
        }
    ...
    def _find_cost_threshold(self, sensitivity_results: List) -> float:
        ...
        for result in sensitivity_results:
            ...
            if 'wfa_results' in result:
                ...
                if wfa_results:
                    avg_pf = sum(r.get('profit_factor', 0) for r in wfa_results) / len(wfa_results)
                    is_viable = avg_pf > 1.0
            elif 'drawdown_risk' in result:
                is_viable = result['drawdown_risk']['is_strategy_viable']
