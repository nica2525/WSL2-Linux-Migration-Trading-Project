...
class PurgedEmbargoedWFA:
    ...
    def __init__(self, data, config, strategy_config):
        ...
        """
        self.data = (
            TimeSeriesData(data) if not isinstance(data, TimeSeriesData) else data
        )
        self.config = config
    ...
    def generate_folds(self):
        ...
        print(
            f"📅 データ期間: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
        )
        ...
        while True:
            ...
            print(
                f"   IS:  {current_is_start.strftime('%Y-%m-%d')} to {actual_is_end.strftime('%Y-%m-%d')} ({fold['is_days']}日)"
            )
            print(
                f"   OOS: {oos_start.strftime('%Y-%m-%d')} to {oos_end.strftime('%Y-%m-%d')} ({fold['oos_days']}日)"
            )
...
class StatisticalValidator:
    ...
    def calculate_oos_consistency(self):
        ...
        oos_returns = [
            fold.get("oos_return", 0) for fold in self.results if "oos_return" in fold
        ]
...
def create_sample_data():
    ...
    for _i in range(400000):  # 約4年分のM5バー数
        # 価格をランダムウォーク
...
def main():
    ...
    print("\n✅ プロトタイプ実行完了！")
    print("   このシステムは47EA失敗の根本原因（情報リーク）を解決します。")
