...
class WFACompliantOptimizer:
    ...
    def _execute_adaptive_backtest(
        self, detector: MarketRegimeDetector, regimes: pd.Series
    ) -> Dict:
        """環境適応型バックテスト（学習期間のみ）"""
        trades = []
        balance = 100000
        ...
        for i in range(len(self.learning_data)):
            ...
            if position is None and signal != "HOLD":
                ...
                position = {
                    ...
                    "stop_loss": strategy.calculate_stop_loss(next_open, signal),
                    "take_profit": strategy.calculate_take_profit(next_open, signal),
                ...
                if exit_signal:
                    ...
                    trades.append({"pnl": adjusted_pnl, "exit_reason": exit_reason})
        ...
        sharpe_ratio = (
            np.mean(pnl_series) / np.std(pnl_series) if np.std(pnl_series) > 0 else 0
        )
    ...
    def objective_function(self, params: List[float]) -> float:
        ...
        score = (
            ...
            + max(sharpe, -2.0) * 0.3  # PF（対数変換で安定化）
            + trade_penalty * 0.1  # シャープレシオ  # 取引数確保
        )
...
class CorrectedAdaptiveWFASystem:
    ...
    def extract_period_data(
        self, data: pd.DataFrame, start: str, end: str
    ) -> pd.DataFrame:
        """期間データ抽出"""
    ...
    def run_corrected_wfa(self) -> Dict:
        ...
        for fold_config in self.wfa_config:
            ...
            learning_data = self.extract_period_data(
                data,
                fold_config["learning_period"][0],
                fold_config["learning_period"][1],
            )
        ...
        self.results = {"corrected_wfa": fold_results, "summary": summary}
    ...
    def _evaluate_on_test_period(
        self, params: np.ndarray, test_data: pd.DataFrame
    ) -> Dict:
        """テスト期間での評価（パラメータ固定）"""
