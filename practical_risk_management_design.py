...
class PracticalRiskManager:
    ...
    def should_enter_trade(
        self, market_data: Dict, current_time: datetime
    ) -> tuple[bool, str]:
        """実用的取引判定"""

        # 1. 異常相場チェック（最優先）
        anomaly_check = self.anomaly_detector.check_market_anomaly(
...
def test_practical_risk_management():
    ...
    for scenario in test_scenarios:
        can_trade, reason = risk_manager.should_enter_trade(
            scenario["market_data"], scenario["current_time"]
        )
