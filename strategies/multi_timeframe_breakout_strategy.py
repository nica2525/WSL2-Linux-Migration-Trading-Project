...
class MultiTimeframeData:
    ...
    def _aggregate_to_h4(self):
        ...
        for bar in self.h1_data:
            ...
            h4_key = bar["datetime"].replace(
                hour=h4_hour, minute=0, second=0, microsecond=0
            )
    ...
    def get_aligned_data(self, target_datetime):
        ...
        return {"M5": m5_bar, "H1": h1_bar, "H4": h4_bar}
...
class MultiTimeframeBreakoutStrategy:
    ...
    def generate_signal(self, mtf_data, current_datetime):
        ...
        h4_index = (
            len(
                [bar for bar in mtf_data.h4_data if bar["datetime"] <= current_datetime]
            )
            - 1
        )
        ...
        h1_index = (
            len(
                [bar for bar in mtf_data.h1_data if bar["datetime"] <= current_datetime]
            )
            - 1
        )
        ...
        if current_price > h4_high + min_break and current_price > h1_high + min_break:
            signal = {
                "type": "long",
        ...
        elif current_price < h4_low - min_break and current_price < h1_low - min_break:
            signal = {
                "type": "short",
                "datetime": current_datetime,
                "entry_price": current_price,
...
def main():
    ...
    print(
        f"   IS結果: PF={is_result['profit_factor']:.3f}, 取引数={is_result['total_trades']}"
    )

    # OOS期間バックテスト
    oos_result = strategy.backtest(mtf_data, is_end_date, raw_data[-1]["datetime"])
    print(
        f"   OOS結果: PF={oos_result['profit_factor']:.3f}, 取引数={oos_result['total_trades']}"
    )

    # Stage1判定
    stage1_pass = (
        oos_result["profit_factor"] >= 1.1 and oos_result["total_trades"] >= 100
    )

    print(f"\n🎯 Stage1判定: {'合格' if stage1_pass else '不合格'}")
    print(
        f"   OOS PF: {oos_result['profit_factor']:.3f} ({'≥1.1' if oos_result['profit_factor'] >= 1.1 else '<1.1'})"
    )
    print(
        f"   取引数: {oos_result['total_trades']} ({'≥100' if oos_result['total_trades'] >= 100 else '<100'})"
    )
