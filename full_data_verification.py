...
class FullDataVerification:
    ...
    def _load_light_results(self):
        """軽量版結果読み込み"""
        try:
            with open("minimal_wfa_results.json") as f:
                return json.load(f)
    ...
    def run_full_verification(self):
        ...
        print(
            f"   期間: {raw_data[0]['datetime'].strftime('%Y-%m-%d')} - {raw_data[-1]['datetime'].strftime('%Y-%m-%d')}"
        )
        ...
        self._save_verification_results(
            full_wfa_results, comparison_results, reliability_assessment
        )
    ...
    def _execute_full_wfa(self, raw_data):
        ...
        for i, fold in enumerate(folds, 1):
            print(
                f"   フォールド{i}: {fold['is_start'].strftime('%Y-%m')} - {fold['oos_end'].strftime('%Y-%m')}"
            )
            ...
            try:
                ...
                is_data = [
                    bar
                    for bar in raw_data
                    if fold["is_start"] <= bar["datetime"] <= fold["is_end"]
                ]
                oos_data = [
                    bar
                    for bar in raw_data
                    if fold["oos_start"] <= bar["datetime"] <= fold["oos_end"]
                ]
                ...
                # OOS期間
                oos_result = strategy.backtest(
                    oos_mtf, fold["oos_start"], fold["oos_end"]
                )
                ...
                print(
                    f"     IS: PF={is_result['profit_factor']:.3f}, 取引={is_result['total_trades']}"
                )
                print(
                    f"     OOS: PF={oos_result['profit_factor']:.3f}, 取引={oos_result['total_trades']}"
                )
    ...
    def _compare_results(self, full_results):
        ...
        for i in range(min(len(light_wfa), len(full_wfa))):
            ...
            comparison = {
                ...
                "trades_ratio": full_fold["oos_trades"] / light_fold["oos_trades"]
                if light_fold["oos_trades"] > 0
                else 0,
            }
            ...
            print(f"   フォールド{i+1}:")
            print(
                f"     PF: {light_fold['oos_pf']:.3f} → {full_fold['oos_pf']:.3f} (差: {comparison['pf_difference']:+.3f})"
            )
            print(
                f"     取引: {light_fold['oos_trades']} → {full_fold['oos_trades']} (倍率: {comparison['trades_ratio']:.1f}x)"
            )
        ...
        overall_comparison = {
            ...
            "pf_consistency": abs(full_stats["avg_oos_pf"] - light_stats["avg_oos_pf"])
            / light_stats["avg_oos_pf"],
        ...
        print("\n   全体比較:")
        print(
            f"     平均PF: {light_stats['avg_oos_pf']:.3f} → {full_stats['avg_oos_pf']:.3f}"
        )
        print(f"     PF一貫性: {overall_comparison['pf_consistency']:.1%} 誤差")
        print(f"     p値: {light_stats['p_value']:.3f} → {full_stats['p_value']:.3f}")
        print(
            f"     統計的有意性: {'維持' if full_stats['statistical_significance'] else '失失'}"
        )
    ...
    def _calculate_full_stats(self, full_results):
        ...
        if len(oos_returns) > 1:
            ...
            variance = sum((r - mean_return) ** 2 for r in oos_returns) / (
                len(oos_returns) - 1
            )
            std_error = (
                math.sqrt(variance / len(oos_returns)) if variance > 0 else 0.001
            )
